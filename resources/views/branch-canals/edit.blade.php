    ...
    <div class="side-app">
        <div class="main-container container-fluid">
            <div class="page-header">
                <h1 class="page-title">Edit branchCanal</h1>
                <div>
                    <ol class="breadcrumb">
            ...
            <div class="row">
                <div class="col-md-8 col-lg-6">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="{{ route('branch-canals.update', $branchCanal) }}" method="post" id="branch-canal-form">
                                @csrf
                                @method('PUT')
                                <div class="mb-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="branch_canal_id" id="branch_canal_id" class="form-select @error('branch_canal_id') is-invalid @enderror" required>
                                        ...
                                        @foreach ($branchCanals as $branchCanal)
                                            <option value="{{ $branchCanal->id }}" @selected(old('branch_canal_id', $branchCanal->branch_canal_id) == $branchCanal->id)>{{ $branchCanal->name }}</option>
                                        @endforeach
                                    </select>
                                    <small class="text-muted">Changing barrage reloads branchCanals via AJAX.</small>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="name" id="name" value="{{ old('name', $branchCanal->name) }}" class="form-control @error('name') is-invalid @enderror" required maxlength="255">
                                    @error('name')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
...
$(function () {
    ...
    @if (old('barrage_id') !== null)
        loadSubCanals(@json(old('barrage_id')), @json(old('branch_canal_id', $branchCanal->branch_canal_id)));
    @endif
});
</script>
@endsection
